...
use Lobage\Planify\Exceptions\UsageDenied;
use Lobage\Planify\Services\Period;
use Lobage\Planify\Traits\BelongsToPlan;
use Lobage\Planify\Traits\HasFeatures;
use Lobage\Planify\Traits\HasGracePeriod;
use Lobage\Planify\Traits\HasGracePeriodUsage;
use Lobage\Planify\Traits\HasPricing;
use Lobage\Planify\Traits\HasSubscriptionPeriodUsage;
use Lobage\Planify\Traits\HasTrialPeriodUsage;
use Lobage\Planify\Traits\HasSchedules;
use Carbon\Carbon;
use DB;
...
class PlanSubscription extends Model
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->setTable(config('planify.tables.plan_subscriptions'));
    }
    ...
    public function getRules(): array
    {
        return [
            'tag' => [
                ...
                'alpha_dash',
                'max:150',
                Rule::unique(config('planify.tables.plan_subscriptions'))->where(function ($query) {
            ...
            'plan_id' => 'required|exists:' . config('planify.tables.plans') . ',id',
    ...
    {
        return $this->hasMany(config('planify.models.plan_subscription_feature'), 'plan_subscription_id', 'id');
    }
    ...
    public function usage(): \Illuminate\Database\Eloquent\Relations\HasManyThrough
    {
        return $this->hasManyThrough(
            config('planify.models.plan_subscription_usage'),
            config('planify.models.plan_subscription_feature'),
    ...
    {
        if (!$ignoreFallback && config('planify.fallback_plan_tag')) { // Do not cancel if a fallback plan is set
            $plan = Plan::getByTag(config('planify.fallback_plan_tag'));
            if (!$plan) {
                throw new UnexpectedValueException('Fallback plan ' . config('planify.fallback_plan_tag') . ' does not exist.');
            }
     ...
     * @param int $uses
     *
     * @return \Lobage\Planify\Models\PlanSubscriptionUsage|null
     */
