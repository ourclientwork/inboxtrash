...
use Lobage\Planify\Exceptions\InvalidPlanSubscription;
use Lobage\Planify\Models\Plan;
use Lobage\Planify\Models\PlanCombination;
use Lobage\Planify\Models\PlanSubscription;
use Lobage\Planify\Services\SubscriptionPeriod;
use Carbon\Carbon;
...
trait HasSubscriptions
{
    ...
    {
        return $this->morphMany(config('planify.models.plan_subscription'), 'subscriber', 'subscriber_type', 'subscriber_id');
    }
    ...
    public function subscription(?string $subscriptionTag = null)
    {
        ...
        $subscriptionTag = $subscriptionTag ?? config('planify.main_subscription_tag');
     ...
     *
     * @return \Lobage\Planify\Models\PlanSubscription|null
     */
    public function subscribedPlans()
    {
        $planIds = $this->subscriptions->reject->isInactive()->pluck('plan_id')->unique();

        return app(config('planify.models.plan'))->whereIn('id', $planIds)->get();
    }
     ...
     * @param \Lobage\Planify\Models\Plan|\Lobage\Planify\Models\PlanCombination $planCombination Plan pricing and invoice data
    ...
    {
        $tag = $tag ?? config('planify.main_subscription_tag');
