...
@section('content')
    <x-breadcrumb title='Edit Plan' col="col col-xl-8 col-xxl-8" backTo="{{ route('admin.plans.index') }}" />
    <div>
                            ...
                            <form action="{{ route('admin.plans.update', $plan->id) }}" method="POST">
                                ...
                                <div class="row row-cols-1 g-3">
                                    ...
                                    <div class="col col-sm-6">
                                        <x-input name='trial' value="{{ $plan->trial_period }}" required label="Trial"
                                            step="1" min="0" type='number' />
                                    </div>

                                    <div class="col col-sm-6">
                                        <x-input type='number' required name='price' step="0.01" min="0"
                                            label="Price" placeholder="9.99" value="{{ $plan->price }}" />
                                    </div>

                                    <div class="col col-sm-6">
                                        <x-label name="Plan Type" for="type" />
                                        <select class="select-input" name="type" id="type">
                                            <option {{ $plan->invoice_interval == 'month' ? 'selected' : '' }}
                                                value="month">{{ __('Monthly') }}
                                            </option>
                                            <option
                                                {{ $plan->invoice_interval == 'year' && $plan->is_lifetime == 0 ? 'selected' : '' }}
                                                value="year">{{ __('Yearly') }}
                                            </option>
                                            <option {{ $plan->is_lifetime == 1 ? 'selected' : '' }} value="lifetime">
                                                {{ __('Lifetime') }}
                                            </option>
                                        </select>
                                        <x-error name="type" />
                                    ...
                                    <div class="col col-sm-6">
                                        ...
                                        <select class="select-input" name="visible" id="visible">
                                            <option {{ $plan->is_active == '1' ? 'selected' : '' }} value="1">
                                                {{ __('Public') }}
                                            </option>
                                            <option {{ $plan->is_active == '0' ? 'selected' : '' }} value="0">
                                                {{ __('Unlisted') }}
                                            </option>
                                        </select>
                                    ...
                                    <div class="col col-md-6 ">
                                        <x-input type='number' name='history_position' min="0" placeholder="0"
                                            label="Position"
                                            value="{{ $plan->getFeatureByTag('history')->sort_order }}" />
                                    </div>
