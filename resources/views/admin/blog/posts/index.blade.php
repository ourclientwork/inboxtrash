...
@section('content')
    <x-breadcrumb title='Posts' GoTo="{{ route('admin.blog.posts.create') }}"/>
    ...
    <div class="box p-3">
        <div class="table-header mb-3">
            <div class="row row-cols-auto flex-row-reverse align-items-center  g-2">
                ...
                <div class="col">
                    <form action=" {{ route('admin.blog.posts.index') }}" class="select2-sm" method="GET">
                ...
                <table class="table table-hover align-middle">
                    ...
                    @forelse  ($posts as $post)
                        <tr>
                            ...
                            <td class="text-center">
                                <a
                                    href="{{ route('admin.blog.posts.edit', $post->id) }}">{{ Str::limit($post->title, 40) }}</a>
                            </td>
                            <td class="text-center">
                                <a href="{{ route('admin.settings.languages.translate', $post->lang) }}">
                                    {{ $post->lang }}
                                </a>
                            </td>
                            <td class="text-center">
                                <a href="{{ route('admin.blog.categories.edit', $post->category->id) }}">
                                    {{ $post->category->name }}
                                </a>
                            </td>
                            <td class="text-center">
                                <span class="badge bg-dark">{{ $post->views }}</span>
                            </td>
                            <td class="text-center">
                                    <span class="badge bg-{{ $post->getStatusColor() }}">
                                        {{ $post->getStatusLabel() }}
                                    </span>
                            </td>
                            <td class="text-center">
                            ...
                            <td class="d-flex justify-content-center">
                                <div class="d-flex gap-2">
                                    ...
                                    <a target="_blank" href="{{ route('posts', $post->slug) }}"
                                    ...
                                    <button class="btn btn-danger cp-x-2" data-bs-toggle="modal"
                                            ...
                                            data-action="{{ route('admin.blog.posts.destroy', $post->id) }}">
        ...
        <div class="table-footer mt-3">
            <div class="row row-cols-auto justify-content-between align-items-center g-3">
                <div class="col">
                    <span>
                        {{ __('Showing') }} {{ $posts->firstItem() }} {{ __('to') }}
                        {{ $posts->lastItem() }} {{ __('of') }}
