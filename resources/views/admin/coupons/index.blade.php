...
@section('content')
    <x-breadcrumb title='Coupons' GoTo="{{ route('admin.coupons.create') }}" />
    ...
    <div class="box p-3">
                        ...
                        <div class="drop-down-menu">
                            <div class="filters-box">
                                <div class="filters-box-header d-flex justify-content-between gap-2">
                                    ...
                                    <a href="{{ route('admin.coupons.index') }}" class="filters-box-reset btn btn-reset">
                ...
                <table class="table table-hover align-middle">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">{{ __('Usage / Limit') }}</th>
                            <th class="text-center">{{ __('Created at') }}</th>
                            <th class="text-center">{{ __('Expired at') }}</th>
                    ...
                    <tbody>
                        @forelse  ($coupons as $coupon)
                            <tr>
                                ...
                                <td class="text-center">
                                    {{ $coupon->usage }} / {{ $coupon->limit == -1 ? '∞' : $coupon->limit }}
                                </td>
                                <td class="text-center">
                                    {{ toDate($coupon->created_at) }}
                                </td>

                                <td class="text-center">
                                    {{ toDate($coupon->expired_at) }}
                                </td>
                                <td class="d-flex justify-content-center">
                                    <div class="d-flex gap-2">
                                        ...
                                        <button class="btn btn-danger cp-x-2" data-bs-toggle="modal"
                                            data-bs-target="#deleteModal" data-id="{{ $coupon->id }}"
        ...
        <div class="table-footer mt-3">
            <div class="row row-cols-auto justify-content-between align-items-center g-3">
                <div class="col">
                    <span>
                        {{ __('Showing') }} {{ $coupons->firstItem() }} {{ __('to') }}
                        {{ $coupons->lastItem() }} {{ __('of') }}
                        {{ $coupons->total() }} {{ __('entries') }}
                    </span>
                </div>
                <div class="col">
