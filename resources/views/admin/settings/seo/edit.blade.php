    ...
    <div class="settings">
        ...
        <div class="settings-content w-100">
            <div class="box">
                ...
                <form action="{{ route('admin.settings.seo.update', $seo->id) }}" method="POST"
                    ...
                    <div class="seo-form pt-3 mt-3">
                        <div class="row row-cols-1 g-3">
                            <div class="col">
                                ...
                                <select class="select-input" hidden name="lang">
                                    @foreach (getAllLanguages() as $lang)
                                        ...
                                        @else
                                            <option {{ $seo->lang == $lang->code ? 'selected' : '' }}
                            ...
                            <div class="col">
                                <x-input name='title' required label="Title" value="{{ $seo->title }}" />
                            </div>

                            <div class="col">
                                <label class="form-label">{{ __('Description') }}</label>
                                <textarea type="text" class="form-control" rows="5" name="description">{{ $seo->description }}</textarea>
                                <x-error name="description" />
                            </div>

                            <div class="col">
                                <div class="tagsinput tagsinput-md">
                                    <x-input data-role="tagsinput" required name="keyword" value="{{ $seo->keyword }}"
                                        label="keywords" />
                                </div>
                            </div>

                            <div class="col">
                                <x-input name='author' label="Author Name" value="{{ $seo->author }}" />
                            </div>
