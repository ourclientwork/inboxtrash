...
@section('content')
    @php
        ...
        $count = translate('Messages', 'general');
        $get_message_feature == 'true' || $get_message_feature == '-1'
            ? ''
            : ($count .= ' (' . $count_messages . '/' . $get_message_feature . ')');
    ...
    <div class="box p-3">
                ...
                <table class="table table-hover align-middle">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">{{ translate('received At') }}</th>
                    ...
                    <tbody>
                        @forelse  ($messages as $message)
                            <tr>
                                <td class="px-3">
                                    <a href="{{ route('view', $message->message_id) }}">
                                ...
                                <td class="text-center">
                                    <button class="btn btn-danger cp-x-2" data-bs-toggle="modal"
                                        ...
                                        data-action="{{ route('messages.destroy', $message->id) }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center empty-svg">
                                    ...
                                    <h4 class="text-muted">{{ __('No Favorite Messages Available') }}</h4>
                                    <p class="text-muted">
                                        {{ __('It looks like there are no favorite messages to display. Please check back later.') }}
                                    </p>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="table-footer mt-3">
            <div class="row row-cols-auto justify-content-between align-items-center g-3">
                <div class="col">
                    <span>
                        {{ translate('Showing') }} {{ $messages->firstItem() }} {{ translate('to') }}
                        {{ $messages->lastItem() }} {{ translate('of') }}
                        {{ $messages->total() }} {{ translate('entries') }}
                    </span>
                </div>
                <div class="col">
