    ...
    <div class="plan box h-100">
        <div class="plan-header">
            <div class="d-flex justify-content-between align-items-center gap-2 mb-3">
                <h6 class="plan-title">{{ translate($plan->name, 'plans') }}</h6>
                @if ($plan->is_featured)
                    <div class="plan-badge">{{ translate('Popular') }}</div>
                @endif
            </div>
            <div class="plan-price d-flex align-items-end mb-2">
                <p class="h1 mb-0">{{ getSetting('currency_symbol') }}{{ $plan->price }}</p>
                <span class="ms-1 mb-2 text-muted">/
                    {{ $plan->is_lifetime == 1 ? translate('Lifetime') : translate(ucfirst($plan->invoice_interval) . 'ly') }}</span>
            </div>
            ...
            <div class="row row-cols-1 g-2">
                <div class="col">
                    @if ($user->subscriptions->count() == 0)
                        <a href="{{ route('checkout', encode_hash($plan->id)) }}" class="btn btn-primary btn-md w-100">
                            {{ $plan->trial_period == 0 ? 'Get Started' : 'Start free trial' }}</a>
                    @else
                        @if ($plan->id != $plan_id)
                            <a href="{{ route('checkout', encode_hash($plan->id)) }}"
